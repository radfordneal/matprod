...
#   if DEBUG_PRINTF
        debug_printf(
         "mat_mat_sub_xrowscols %p %p %p - %d %d %d - %d %d %d %d\n",
                                 x, y, z,   n, k, m,  xrows, zn, xcols, add);
#   endif
...
#               if ALIGN_FORWARD & 8
                {
                    ...
                    _mm_store_sd(z+j+zn,
                                       _mm_add_sd(_mm_mul_sd (S1, cast128(B21)),
                    ...
                    if (ALIGN >= 32 && (n & 3) == 0) { /* +n keeps ptr aligned*/
                        ...
                        do {
                            ...
                            _mm256_storeA_pd(z+j, _mm256_add_pd (_mm256_add_pd(
                                    _mm256_loadA_pd(z+j),_mm256_mul_pd(S1,B11)),
                                                       _mm256_mul_pd(S2,B12)));
                            _mm256_storeu_pd(z+j+zn,_mm256_add_pd(_mm256_add_pd(
                                  _mm256_loadu_pd(z+j+zn),
                                                       _mm256_mul_pd(S1,B21)),
                                                       _mm256_mul_pd(S2,B22)));
                            j += 4;
                        } while (j <= xrows-4);
                    ...
                    if (ALIGN >= 16 && (n & 1) == 0) { /* +n keeps ptr aligned*/
                        do {
                            ...
                            _mm_storeA_pd(z+j,_mm_add_pd (
                                               ...
                                               _mm_mul_pd(S1,B11)),
                                               _mm_mul_pd(S2,B12)));
                            _mm_storeA_pd(z+j+zn,_mm_add_pd (
                                               _mm_add_pd(_mm_loadA_pd(z+j+zn),
                                               _mm_mul_pd(S1,B21)),
                                               _mm_mul_pd(S2,B22)));
                            S1 = _mm_loadA_pd(xx+j+2);
                            ...
                            _mm_storeA_pd(z+j+2,_mm_add_pd (
                                               ...
                                               _mm_mul_pd(S1,B11)),
                                               _mm_mul_pd(S2,B12)));
                            _mm_storeA_pd(z+j+zn+2,_mm_add_pd (
                                              _mm_add_pd(_mm_loadA_pd(z+j+zn+2),
                                              _mm_mul_pd(S1,B21)),
                                              _mm_mul_pd(S2,B22)));
                            j += 4;
                        } while (j <= xrows-4);
                        ...
                        do {
                            ...
                            _mm_storeA_pd(z+j,_mm_add_pd (
                                               ...
                                               _mm_mul_pd(S1,B11)),
                                               _mm_mul_pd(S2,B12)));
                            _mm_storeu_pd(z+j+zn,_mm_add_pd (
                                               _mm_add_pd(_mm_loadu_pd(z+j+zn),
                                               _mm_mul_pd(S1,B21)),
                                               _mm_mul_pd(S2,B22)));
                            S1 = _mm_loadA_pd(xx+j+2);
                            ...
                            _mm_storeA_pd(z+j+2,_mm_add_pd (
                                               ...
                                               _mm_mul_pd(S1,B11)),
                                               _mm_mul_pd(S2,B12)));
                            _mm_storeu_pd(z+j+zn+2,_mm_add_pd (
                                              _mm_add_pd(_mm_loadu_pd(z+j+zn+2),
                                              _mm_mul_pd(S1,B21)),
                                              _mm_mul_pd(S2,B22)));
                            j += 4;
                        } while (j <= xrows-4);
                    ...
                    if (ALIGN >= 32 && (n & 3) == 0) { /* +n keeps ptr aligned*/
                        do {
                            ...
                            _mm256_storeA_pd (z+j, _mm256_add_pd (
                                                        _mm256_loadA_pd(z+j), 
                                                        _mm256_mul_pd(S,B1)));
                            _mm256_storeA_pd (z+j+zn, _mm256_add_pd(
                                                   _mm256_loadA_pd(z+j+zn),
                                                   _mm256_mul_pd(S,B2)));
                            j += 4;
                        } while (j <= xrows-4);
...
{
#   if DEBUG_PRINTF
        debug_printf("trans1_sub_xrows %p %p %p - %d %d %d - %d %d %d\n",
...
{
#   if DEBUG_PRINTF
        debug_printf("trans1_sub_xrowscols %p %p %p - %d %d %d - %d %d %d %d\n",
                                            x, y, z,   n, k, m,  sym, add, 
                                                                 xrows, xcols);
#   endif
...
{
#   if DEBUG_PRINTF
        debug_printf("trans2_sub_xrows %p %p %p - %d %d %d - %d %d %d\n",
...
#   if DEBUG_PRINTF
        debug_printf(
         "trans2_sub_xrowscols %p %p %p - %d %d %d - %d %d %d %d %d\n",
                                x, y, z,   n, k, m,  sym, xrows, yrows,
                                                     xcols, add);
#   endif
...
#       else  /* non-SIMD code */
        {
            ...
            /* Store the two sums in s[0] and s[1] in the last column
               of the result. */

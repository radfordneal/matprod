...
#           if CAN_USE_SSE2
                ...
                while (k2 > 1) {
                    ...
                    R = _mm_loadAA_pd(r+2);
                    S0 = _mm_add_pd (_mm_mul_pd (R, _mm_set1_pd(q[k])), S0);
                    S1 = _mm_add_pd (_mm_mul_pd (R, _mm_set1_pd(q[k+1])), S1);
                    r += 4;
                    q += 2;
                    k2 -= 2;
                }
...
#           endif
    ...
    while (m2 > 1) {
        ...
        double *zs = sym ? z+m-m2 : z;   /* Where to start storing sums */
